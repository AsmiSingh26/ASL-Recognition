...
# ── Header ──
st.markdown('<div class="main-header">🤟 ASL Recognition System</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-header">American Sign Language Alphabet Recognition using Deep Learning</div>', unsafe_allow_html=True)
...
# ── Tabs ──
tab1, tab2, tab3 = st.tabs(["📸 Image Upload", "📝 Word Builder", "📊 Model Info"])
...
with tab1:
    ...
    with col_upload:
        ...
        if uploaded_file:
            ...
            st.image(cv2.cvtColor(img.copy(), cv2.COLOR_BGR2RGB),
    ...
    with col_result:
        if uploaded_file is not None:
            ...
            if error:
                st.error(f"❌ {error}")
                st.info("💡 Ensure hand is clearly visible with good lighting")
            else:
                ...
                <div class="result-box">
                    ...
                    <div class="confidence-text">Confidence: {conf*100:.1f}% — {status}</div>
                </div>
                """, unsafe_allow_html=True)
                ...
                st.image(cv2.cvtColor(img, cv2.COLOR_BGR2RGB),
                         caption="Landmarks", use_container_width=True)
...
with tab2:
    ...
    with col_w1:
        word_upload = st.file_uploader("Upload next letter", type=["jpg", "jpeg", "png"], key="word_uploader")
        ...
        if word_upload:
            file_bytes = np.asarray(bytearray(word_upload.read()), dtype=np.uint8)
            ...
            st.image(cv2.cvtColor(img_w, cv2.COLOR_BGR2RGB),
                     caption=f"Detected: {label}" if not error else "No hand",
                     use_container_width=True)

    with col_w2:
        st.markdown("#### 📝 Current Word")
        word_display = st.session_state.word if st.session_state.word else "..."
        st.markdown(f'<div class="word-buffer">{word_display}</div>', unsafe_allow_html=True)

        st.markdown("#### 📜 Letter History")
        if st.session_state.history:
            st.code(" → ".join(st.session_state.history[-10:]))
        else:
            st.code("No letters added yet")

        if st.session_state.word:
            st.download_button("💾 Download Word", data=st.session_state.word,
                               file_name="asl_word.txt", mime="text/plain")
...
Dense(29) + Softmax
    ...
    with col_m2:
        ...
        st.markdown("""
- **M vs N** most confused pair
- **B, C, L, Y** — perfect classification
- Filtering bad detections improved accuracy by **23%**
